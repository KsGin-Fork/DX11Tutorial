...
bool FireShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	char fire_vs[] = "./fireVertexShader.hlsl";
	char fire_ps[] = "./firePixelShader.hlsl";
	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, fire_vs, fire_ps);
	if(!result)
	{
		return false;
	}
...
bool FireShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
							 DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* fireTexture, 
							 ID3D11ShaderResourceView* noiseTexture, ID3D11ShaderResourceView* alphaTexture, float frameTime,
							 DirectX::XMFLOAT3 scrollSpeeds, DirectX::XMFLOAT3 scales, DirectX::XMFLOAT2 distortion1, DirectX::XMFLOAT2 distortion2,
							 DirectX::XMFLOAT2 distortion3, float distortionScale, float distortionBias)
{
	bool result;
...
bool FireShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* psFilename)
{
	HRESULT result;
...
void FireShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, CHAR* shaderFilename)
{
	char* compileErrors;
...
bool FireShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
							 DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* fireTexture, 
							 ID3D11ShaderResourceView* noiseTexture, ID3D11ShaderResourceView* alphaTexture, float frameTime,
							 DirectX::XMFLOAT3 scrollSpeeds, DirectX::XMFLOAT3 scales, DirectX::XMFLOAT2 distortion1, DirectX::XMFLOAT2 distortion2,
							 DirectX::XMFLOAT2 distortion3, float distortionScale, float distortionBias)
