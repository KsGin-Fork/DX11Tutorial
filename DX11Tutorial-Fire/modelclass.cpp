...
bool ModelClass::Initialize(ID3D11Device* device, char *modelFilename ,char* texture1Filename, char* texture2Filename , char* texture3Filename)
{
	bool result;
	...
	result = LoadTexture(device, texture1Filename , texture2Filename , texture3Filename);
	if(!result)
	{
		return false;
	}
...
bool ModelClass::LoadTexture(ID3D11Device* device, CHAR* filename1 , CHAR* filename2 , CHAR* filename3)
{
	...
	result = m_Texture1->Initialize(device, filename1);
	if(!result)
	{
		return false;
	}
	...
	// Initialize the texture object.
	result = m_Texture3->Initialize(device, filename3);
	if(!result)
	{
		return false;
	}
