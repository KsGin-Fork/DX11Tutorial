...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	CHAR texture_hs[] = "./textureHullShader.hlsl";
	CHAR texture_ds[] = "./textureDomainShader.hlsl";
	CHAR texture_ps[] = "./texturePixelShader.hlsl";
	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, texture_vs, texture_hs , texture_ds , texture_ps);
	if(!result)
	{
		return false;
	}
...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
								DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture , float tessellationAmount)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture , tessellationAmount);
	if(!result)
	{
		return false;
	}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* hsFilename , CHAR* dsFilename , CHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(hsFilename, NULL, NULL, "TextureHullShader", "hs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	result = D3DX11CompileFromFile(dsFilename, NULL, NULL, "TextureDomainShader", "ds_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	// Setup the description of the tessellation dynamic constant buffer that is in the vertex shader.
											 ...
											 DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture , float tessellationAmount)
{
	...
	TessellationBufferType* dataPtr1;
	...
	dataPtr1 = (TessellationBufferType*)mappedResource.pData;

	dataPtr1->tessellationAmount = tessellationAmount;
	dataPtr1->padding = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
