...
bool GlassShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	CHAR texture_vs[] = "./glassVertexShader.hlsl";
	CHAR texture_ps[] = "./glassPixelShader.hlsl";
...
bool GlassShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture , normalTexture , refractionTexture , refractionScale);
	if(!result)
	{
		return false;
	}
...
bool GlassShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd,
	...
	// Setup the description of the glass dynamic constant buffer that is in the vertex shader.
...
bool GlassShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix,
